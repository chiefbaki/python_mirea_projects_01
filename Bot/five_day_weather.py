...
class TodayWeather:
    ...
    lat = 55.7504461
    lon = 37.6174943
    api_key = f'https://api.openweathermap.org/data/2.5/forecast?lat={lat}&lon={lon}&appid={token_id}&units=metric&lang=ru'
    ...
    def __init__(self):
        ...
        # Температура
        self.morning_temp_max = str(floor(int(self._info['list'][0]['main']['temp_max'])))  # 09:00
        self.afternoon_temp_max = str(floor(int(self._info['list'][2]['main']['temp_max'])))  # 15:00
        self.evening_temp_max = str(floor(int(self._info['list'][3]['main']['temp_max'])))  # 18:00
        self.night_temp_max = str(floor(int(self._info['list'][4]['main']['temp_max'])))  # 21:00
        # Иконки
        self.morning_icon = self._info['list'][0]['weather'][0]['icon']
        self.afternoon_icon = self._info['list'][2]['weather'][0]['icon']
        self.evening_icon = self._info['list'][3]['weather'][0]['icon']
        self.night_icon = self._info['list'][4]['weather'][0]['icon']

    def morning(self):
        morning_temp = str(int(self._info['list'][0]['main']['temp_min'])) + '-' + str(int(self._info['list'][0]['main']['temp_max']))
        morning_desc = self._info['list'][0]['weather'][0]['description']
        morning_humid = self._info['list'][0]['main']['humidity'] # Влажность
        morning_pressure = str(int(float(self._info['list'][7]['main']['pressure']) / 1.33)) # Давление
        morning_wind_speed = self._info['list'][0]['wind']['speed'] # Ветер
    ...
    def afternoon(self):
        afternoon_temp = str(int(self._info['list'][2]['main']['temp_min'])) + '-' + str(
            int(self._info['list'][2]['main']['temp_max']))
        afternoon_desc = self._info['list'][2]['weather'][0]['description']
        afternoon_humid = self._info['list'][2]['main']['humidity']
        afternoon_pressure = str(int(float(self._info['list'][2]['main']['pressure']) / 1.33))
        afternoon_wind_speed = self._info['list'][2]['wind']['speed']
    ...
    def evening(self):
        evening_temp = str(int(self._info['list'][3]['main']['temp_min'])) + '-' + str(
            int(self._info['list'][3]['main']['temp_max']))
        evening_desc = self._info['list'][3]['weather'][0]['description']
        evening_humid = self._info['list'][3]['main']['humidity']
        evening_pressure = str(int(float(self._info['list'][3]['main']['pressure']) / 1.33))
        evening_wind_speed = self._info['list'][3]['wind']['speed']
    ...
    def night(self):
        night_temp = str(int(self._info['list'][4]['main']['temp_min'])) + '-' + str(
            int(self._info['list'][4]['main']['temp_max']))
        night_desc = self._info['list'][4]['weather'][0]['description']
        night_humid = self._info['list'][4]['main']['humidity']
        night_pressure = str(int(float(self._info['list'][4]['main']['pressure']) / 1.33))
        night_wind_speed = self._info['list'][4]['wind']['speed']
    ...
    def show_pic(self):
        image = Image.new('RGBA', (400, 100), 'brown')
...
class TomorrowWeather:
    ...
    def morning(self):
        morning_temp = str(float(self._info['list'][7]['main']['temp_min']) - 1) + '-' + str(int(self._info['list'][7]['main']['temp_max']))
    ...
    def afternoon(self):
        afternoon_temp = str(float(self._info['list'][9]['main']['temp_min']) - 1) + '-' + str(int(self._info['list'][9]['main']['temp_max']))
    ...
    def evening(self):
        evening_temp = str(float(self._info['list'][10]['main']['temp_min']) - 1) + '-' + str(int(self._info['list'][10]['main']['temp_max']))
    ...
    def night(self):
        ...
        night_wind_speed = self._info['list'][11]['wind']['speed']
        night_wind_slug = self._get_wind_slug(float(night_wind_speed)).lower()
        night_wind_deg_slug = self._get_wind_deg_slug(night_wind_speed)
...
class Weather5Day:
    ...
    @property
    def get_info(self):
        return f'/ {self.first_day}°C // {self.second_day}°C // {self.third_day_}°C\
